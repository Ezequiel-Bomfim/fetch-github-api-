const screen = {
    ...
    renderUser(user) {
        ...
        user.repositories.forEach(repo => repositoriesItens += `<li><a href="${repo.html_url}" target="_blank">${repo.name}  <div class="info-repositories">  <div>🍴${repo.forks}</div>   <div>⭐${repo.stargazers_count}</div>    <div>👀${repo.watchers}</div>    <div>👨‍💻${repo.language ?? "Sem linguagem"}</div></div></a></li>`)
